...
export default function BookableEdit() {
  ...
  return <BookableForm formState={formState} handleSubmit={handleSubmit} handleDelete={handleDelete} />
}
...
function useUpdateBookable() {
  ...
  const queryClient = useQueryClient()
  const mutation = useMutation((item) => editItem(`http://localhost:3001/bookables/${item.id}`, item), {
    onSuccess: (bookable) => {
...
function useDeleteBookable() {
  const navigate = useNavigate()
  const queryClient = useQueryClient()
  const mutation = useMutation((bookable) => deleteItem(`http://localhost:3001/bookables/${bookable.id}`), {
